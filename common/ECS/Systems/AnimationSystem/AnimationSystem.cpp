...
namespace ecs {
    ...
    void AnimationSystem::update(Registry &registry, float deltaTime) {
        ...
        for (auto entityId : entities) {
            auto &animation = registry.getComponent<Animation>(entityId);
            auto &animationSet = registry.getComponent<AnimationSet>(entityId);
            auto &sprite = registry.getComponent<Sprite>(entityId);
